      ...
      <el-form ref="form" :inline="true">
        <template v-for="item in searchList" :key="item.index">
          ...
          <el-form-item :label="item.name" v-if="item.type=='date'">
            <el-date-picker v-model="searchForm[item.key]" :type="item.dateType||'date'" :placeholder="item.placeholder"
              :default-value="item.defaultValue" :editable="item.editable" :format="item.format"
              :value-format="item.valueFormat" :end-placeholder="item.endplaceholder"
        ...
        <template v-for="item in columns" :key="item.index" align='center'>
          ...
          <el-table-column :width="item.width" v-if="item.type=='switch'" align='center'>
            <template #default={row}>
              <el-switch v-model="row[item.property]" :size="item.size" :active-text="item.activeText"
          ...
          <el-table-column :width="item.width" v-if="item.type=='double'" align='center'>
            <template v-for="i in item.childrenList" :key="i.index">
              <el-table-column :label="i.label" :property="i.property" :width="i.width" :align="i.align"
                v-if="i.type=='text'">
                <template #default={row}>
                  <span @click.stop="textClick(row[i.property])">{{row[i.property]}}</span>
                </template>
              </el-table-column>
              <el-table-column v-if="i.type=='img'" :label="i.label" :property="i.property" width="100" align="center">
                <template #default={row}>
                  <el-image :src="row.src" :preview-src-list="[row.src]" style="width:70px;height:20px" />
                </template>
              </el-table-column>
            </template>
          </el-table-column>
          <el-table-column :label="item.label" :property="item.property" :width="item.width" :align="item.align"
            v-if="item.type=='text'">
            <template #default={row}>
              <span @click.stop="textClick(row[item.property])">{{row[item.property]}}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作" align="center" v-if="item.type=='operation'" :fixed="item.fixed">
            <template #default={row}>
              <el-button v-for="i in item.operationList" :key="i.index" :type="i.type" size="small"
                @click.stop="operationClick(row,i.name)" :disabled="row[i.key]">{{i.name}}
              </el-button>
            </template>
          </el-table-column>
        </template>
